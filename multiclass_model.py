...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    model.add(keras.layers.InputLayer(input_shape=(input_length,)))  # Set input shape explicitly
    model.add(keras.layers.Dense(output_length, activation=activation_f))
    return model
...
def evaluate_mnist_model(x_test, y_test, model):
    ...
    """
    y_test = tf.keras.utils.to_categorical(y_test, num_classes=model.output_shape[1])
